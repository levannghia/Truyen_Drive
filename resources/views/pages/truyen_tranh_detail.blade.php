    ...
    <nav aria-label="breadcrumb">
        <ol class="breadcrumb">
            ...
            <li class="breadcrumb-item"><a
                    href="{{ route('danh.muc', $truyenTranh->categories->slug) }}">{{ $truyenTranh->categories->title }}</a></li>
            <li class="breadcrumb-item active" aria-current="page">{{ $truyenTranh->name }}</li>
        </ol>
    </nav>
    <div class="row">
        <div class="col-md-9">
            <div class="row">
                <div class="col-md-3">
                    <img class="image-truyen" src="{{ asset('public/uploads/truyen_tranh/' . $truyenTranh->image) }}" alt="">
                </div>
                <div class="col-md-9">
                    <style>
                        .info-truyen {
                    ...
                    <ul class="info-truyen">
                        <li>Tên truyện: {{ $truyenTranh->name }}</li>
                        <li>Thoi gian dang: {{ $truyenTranh->created_at->diffForHumans() }}</li>
                        <li>Danh muc: <a
                                href="{{ route('danh.muc', $truyenTranh->categories->slug) }}">{{ $truyenTranh->categories->title }}</a>
                        </li>
                        <li>
                        ...
                        @if (isset($chapter_first))
                            ...
                            <li>
                                <a href="{{ route('xem.chapter.truyen.tranh', $chapter_first->slug) }}" class="btn btn-primary">Đọc
                                    online</a>
                            </li>
                        @endif
            ...
            @if (count($chapter) > 0)
                ...
                <ul class="muc-luc-truyen">
                    @foreach ($chapter as $item)
                        <li><a href="{{ route('xem.chapter.truyen.tranh', $item->slug) }}">{{ $item->title }}</a></li>
                    @endforeach
                </ul>
            @else
            ...
            <div class="owl-carousel owl-theme">
                @foreach ($truyenSameCategory as $item)
                    <div class="item">
                        <img src="{{ asset('public/uploads/truyen_tranh/' . $item->image) }}" alt="">
                        <h3>{{ $item->name }}</h3>
        ...
        $(document).ready(function() {
            ...
            function show_wishlist() {
                if (localStorage.getItem('wishlist_truyen') !== null) {
                    ...
                    var i = 0;
                    const id_current = "{{ $truyenTranh->id }}";
            ...
            $('.btn_thich_truyen').click(function() {
                ...
                const id = "{{ $truyenTranh->id }}";
                const name = "{{ $truyenTranh->name }}";
